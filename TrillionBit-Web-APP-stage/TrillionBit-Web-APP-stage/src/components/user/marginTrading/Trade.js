                ...
                {orderDeals.length > 0 ? (
                    <List className="data scrollBody leftTradeData">
                        {(orderDeals.map((orderDeal, index) => <ListItem className={(orderDeal.m || orderDeal.type === 'sell') ? 'tableData red' : 'tableData'} key={index}>
                            <div className="amount" id="trades_price"> {orderDeal.q ? orderDeal.q : orderDeal.amount} </div>
                            <div className="time" id="trades_time"> {moment(parseInt(orderDeal.T ? orderDeal.T : orderDeal.time * 1000)).format('LT')} </div>
                            <div className="price" id="trades_amount"> {activeMarket.stock === 'BTX' ? parseFloat(orderDeal.p ? orderDeal.p : orderDeal.price).toFixed(4) : parseFloat(orderDeal.p ? orderDeal.p : orderDeal.price).toFixed(2)} </div>
                        </ListItem>))}
                    </List>
                ) : (
                    <List className="data scrollBody leftTradeData">
