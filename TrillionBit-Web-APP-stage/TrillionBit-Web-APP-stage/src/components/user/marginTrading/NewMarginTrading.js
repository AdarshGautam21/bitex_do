...
class MarginTrading extends Component {
    ...
    for (let ckey in this.props.wallet.userAssets) {
      if (
        ...
        await this.setState({
          ...
          marketTabValue: this.props.wallet.userAssets[ckey],
        });
      }
    }
  ...
  componentWillReceiveProps = async (nextProps) => {
    ...
    if (availableMarkets.length > 0) {
      ...
      this.setState({
        dispalyMarkets: coinWiseDisplayMarket,
        marketTabValue: this.state.currentUserMarginFiatWallet.coin,
      });
    }
  ...
  convertUserDefaultFiatToCrypto = (marketName, availableBalance) => {
    ...
    if (
      ...
      const cryptoFiatPrice = !isEmpty(this.state.marketLast)
        ? parseFloat(this.state.marketLast[marketName].ask).toFixed(2)
        : 0.0;
  ...
  openTranserDialog = () => {
    this.setState({ transerDialog: true });
  };

  openBorrowDialog = () => {
    this.setState({ borrowDialog: true });
  };
            ...
            >
              {auth.isAuthenticated && (
                <AuthLink
                  ...
                  openTranserDialog={this.openTranserDialog}
                  openBorrowDialog={this.openBorrowDialog}
          ...
          <Grid container>
            <Grid item xs={12} md={2} sm={12} className="leftCol">
              ...
              <AssetsAllocation
                ...
                currentUserFiatWallet={this.state.currentUserMarginFiatWallet}
            ...
            <Grid item xs={12} md={7} sm={12} className="padding0 centerCol">
              <Grid container>
                <Graph
                  ...
                  chartCoin={chartCoin}
                  currentUserCryptoWallet={currentUserMarginFiatWallet}
                  currentUserFiatWallet={currentUserMarginCryptoWallet}
                  trading={trading}
                  marketLast={this.state.marketLast}
                />
            ...
            {auth.isAuthenticated ? (
              <UserTrade
                ...
                trading={trading}
                wallet={wallet}
                currentUserMarginFiatWallet={currentUserMarginFiatWallet}
                currentUserMarginCryptoWallet={currentUserMarginCryptoWallet}
